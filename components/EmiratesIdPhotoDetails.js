...
export default class PassportPhotoDetails extends React.Component {
  ...
  removeImage1 = async () => {
    console.log('1');
    const emirate_id_photo1 = await AsyncStorage.getItem('emirate_id_photo1');
    if(emirate_id_photo1 != null) {
        let images = [...this.state.images]; // make a separate copy of the array
        let index = JSON.parse(emirate_id_photo1);
  ...
  removeImage2 = async () => {
    console.log('2')
    const emirate_id_photo2 = await AsyncStorage.getItem('emirate_id_photo2');
    if(emirate_id_photo2 != null) {
        let images = [...this.state.images]; // make a separate copy of the array
        let index = JSON.parse(emirate_id_photo2);
        this.setState({images: images.filter(val => val.url != index)});
    }
  ...
  componentDidUpdate = async (prevProps, prevState) => {
    ...
    if(prevProps.route.params.image != this.props.route.params.image) {
      if(this.state.image1 == null) {
        await AsyncStorage.setItem('emirate_id_photo1', JSON.stringify(this.props.route.params.image));
        await AsyncStorage.setItem('emirate_id_photo1_base64', JSON.stringify(this.props.route.params.image_base64));
        let d = new Date();
        await AsyncStorage.setItem('emirate_id_photo1_date', JSON.stringify(d.getDate()+'-'+d.getMonth()+'-'+d.getFullYear()+' '+d.getHours()+':'+d.getMinutes()+':'+d.getSeconds()));

        const emirate_id_photo1 = await AsyncStorage.getItem('emirate_id_photo1');
        const emirate_id_photo1_date = await AsyncStorage.getItem('emirate_id_photo1_date');

        if(car_photo1 != null) {
          let images = [...this.state.images];
          images.push({ url: JSON.parse(emirate_id_photo1) });
          this.setState({
            ...this.state,
            image1: JSON.parse(emirate_id_photo1),
            image1_date: JSON.parse(emirate_id_photo1_date),
            images
          });
        }
      }
        ...
        await AsyncStorage.setItem('emirate_id_photo2', JSON.stringify(this.props.route.params.image));
        await AsyncStorage.setItem('emirate_id_photo2_base64', JSON.stringify(this.props.route.params.image_base64));
         let d = new Date();
         await AsyncStorage.setItem('emirate_id_photo2_date', JSON.stringify(d.getDate()+'-'+d.getMonth()+'-'+d.getFullYear()+' '+d.getHours()+':'+d.getMinutes()+':'+d.getSeconds()));

        const emirate_id_photo2 = await AsyncStorage.getItem('emirate_id_photo2');
        const emirate_id_photo2_date = await AsyncStorage.getItem('emirate_id_photo2_date');

        if(emirate_id_photo2 != null) {
          let images = [...this.state.images];
          images.push({ url: JSON.parse(emirate_id_photo2) });
          this.setState({
            ...this.state,
            image2: JSON.parse(emirate_id_photo2),
            image2_date: JSON.parse(emirate_id_photo2_date),
            images
          });
        }
      }
  ...
  componentDidMount = async () => {
    ...
    console.log('date', emirate_id_photo1_date)
  ...
  render() {
    ...
    if(this.state.image1 != null && this.state.image2 != null) {
        car_photo_render = <TouchableOpacity onPress={() => { this.props.navigation.navigate('Thank You', { test: 'Test' }) }} style={{...styles.fab}}>
                            <Text style={styles.fabIcon}><Icon size={ 18 } name='check-circle'/> { I18n.t('CONFIRM') }</Text>
                          </TouchableOpacity>;
    } else {
      if(this.state.image1 == null) {
          car_photo_render = <TouchableOpacity onPress={() => { this.props.navigation.push('Emirates Id Photos', { test: 'Test' }) }} style={{...styles.fab, width: 200}}>
                                <Text style={styles.fabIcon}><Icon size={ 18 } name='camera'/> { I18n.t('EMIRATES ID FRONT') }</Text>
                              </TouchableOpacity>;
      } else if (this.state.image2 == null) {
          car_photo_render = <TouchableOpacity onPress={() => { this.props.navigation.push('Emirates Id Photos', { test: 'Test' }) }} style={{...styles.fab, width: 190}}>
